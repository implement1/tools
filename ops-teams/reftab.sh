...
Help()
{
  ...
  echo "This has been modified from the original but"
  echo "the examples are valid: https://github.com/Reftab/ReftabShell"
  echo
...
if [ -z "$REFTAB_PUBLIC_KEY" ] || [ -z "$REFTAB_SECRET_KEY" ]; then
  echo '!! missing REFTAB_PUBLIC_KEY / REFTAB_SECRET_KEY variables' >&2
  echo '!! check lastpass note "Reftab Keys"' >&2
  exit 1
fi
...
TOKEN=$(printf "%s" "$SIGNATURETOSIGN" | openssl dgst -sha256 -hmac "$REFTAB_SECRET_KEY")
TOKEN=${TOKEN/* }
TOKEN=$(printf "%s" "$TOKEN" | openssl base64 -A)

curl "$URL" \
  -H "x-rt-date: $DATE" \
  -H "authorization: RT $REFTAB_PUBLIC_KEY:$TOKEN" \
